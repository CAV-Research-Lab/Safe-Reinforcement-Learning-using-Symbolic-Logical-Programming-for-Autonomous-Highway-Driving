...
class ScenarioData():
    def __init__(self, map_id, timestep, road_length, road_width, X, Y):
        ...
        self.X = X
        self.Y = Y
        self.recordingMeta = csv.DictReader(open("../HighD/Metas/" + str(map_id) + '_recordingMeta.csv')).__next__()
        self.tracks = csv.DictReader(open("../HighD/Tracks/" + str(map_id) + '_tracks.csv'))
        self.tracksMeta = csv.DictReader(open("../HighD/Statics/" + str(map_id) + '_tracksMeta.csv'))
        self.running = True
        self.timestep = 1 / 25
        self.start_time = 0
        self.t_count = 1
    ...
    def init_scenario(self):
        ...
        for i in range(self.numVehicles):
            self.vehicle_list.append(
                pygame.Color(int(random.random() * 256), int(random.random() * 256), int(random.random() * 256)))
    ...
    def plot_scenario(self, screen):
        ...
        while self.running:
            ...
            with open('prolog files/vehicle_clauses.pl', 'w') as f:
                ...
                EgoFact = f"vehicle(ego, {EgoLane}, {self.agent.x:.4f}, {self.agent.y:.4f}, {self.agent.width:.4f}, {self.agent.height:.4f}, {self.agent.velocity_x:.4f}, {self.agent.velocity_y:.4f}, 100).\n"
                f.write(EgoFact)
                f.close()
...
if __name__ == '__main__':
    ...
    skip_list = [33, 37, 38, 40, 41, 42, 43, 44, 45, 46, 47, 48, 50, 51, 53, 54, 55, 57, 60]

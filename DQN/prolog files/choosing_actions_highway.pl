...
distance(Car1,Car2,D):-vehicle(Car1,_,X1,Y1,_,_,_),
                        vehicle(Car2,_,X2,Y2,_,_,_), 
                        D is ((X1-X2)^2+(Y1-Y2)^2)^0.5.

distanceX(Car1,Car2,D):-vehicle(Car1,_,X1,_,_,_,_),
                        vehicle(Car2,_,X2,_,_,_,_), 
                        D is abs(X1-X2).

distanceY(Car1,Car2,D):-vehicle(Car1,_,_,Y1,_,_,_),
                        vehicle(Car2,_,_,Y2,_,_,_), 
                        D is abs(Y1-Y2).

relativeVelocity(Car1,Car2,RelVel):-vehicle(Car1,_,_,_,Vx1,_,_),
                                    vehicle(Car2,_,_,_,Vx2,_,_),
                                    RelVel is (Vx2-Vx1).

% Adjacent cars
neighboring_cars(Car):-distance(ego, Car, D), D =< 150, Car \= ego.
...
north(Car):-neighboring_cars(Car),
            vehicle(ego,Lane1,Px1,_,_,_,_),
            vehicle(Car,Lane2,Px2,_,_,_,_),
            Lane1 is Lane2,
...
east(Car):-neighboring_cars(Car),
            vehicle(ego,Lane1,_,_,_,_,_),
            vehicle(Car,Lane2,_,_,_,_,_),
...
safe_actions(Action):-vehicle(ego,Lane,_,_,_,_,_),
...
safe_actions1(Action):-vehicle(ego,Lane,_,_,_,_,_),
                    not(Lane is 3),not(Lane is 4),
                    west_is_free, northwest_is_free,
                    Action = left_lane_change.

safe_actions1(Action):-vehicle(ego,Lane,_,_,_,_,_),
                    not(Lane is 1),not(Lane is 6),
                    east_is_free, northeast_is_free,
                    Action = right_lane_change.
...
desired_velocity_x(V_x):-north_is_free,
                        vehicle(ego,Lane,_,_,_,_,_),
                        max_speed(Lane,X),
                        V_x is X,!.

desired_velocity_x(V_x):-north_is_busy,
                        vehicle(ego,Lane,_,_,Vx1,_,_),
                        vehicle(Car,_,_,_,Vx2,_,_),
                        distance(ego,Car,D),
                        safe_distance(D),
                        critical_distance(C),
